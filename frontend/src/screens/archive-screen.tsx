...
export default function ArchiveScreen() {
  ...
  useEffect(()=>{ repository.listBoards().then((items)=>{setBoards(items); if(items[0]) setSelected(items[0].id);}).finally(()=>setLoading(false)); },[]);
  useEffect(()=>{ if(selected) repository.archive(selected).then((items)=>setTasks(items ?? [])); },[selected]);
