...
class MainView {
  constructor(container) {
    ...
    if (yr === 2026) this.todayDoy = Math.floor((today - new Date(2026, 0, 1)) / 86400000) + 1;
    else this.todayDoy = yr < 2026 ? 0 : 366;
    ...
    this.container.innerHTML = `
      <header>
        <div class="user-header">
          ...
          <div class="main-nav">
            <button id="kanban-btn" class="btn-nav">📋 Kanban</button>
      ...
      <div class="legend">
        <div class="legend-item"><div class="swatch sw-past"></div>Pasado sin marcar</div>
        <div class="legend-item"><div class="swatch sw-future"></div>Por venir</div>
        <div class="legend-item"><div class="swatch sw-done"></div>Todo bien</div>
        <div class="legend-item"><div class="swatch sw-partial"></div>A medias</div>
        <div class="legend-item"><div class="swatch sw-miss"></div>Sin cumplir</div>
      </div>
  ...
  renderGrid(clicks, onDayClick) {
    ...
    for (let i = 1; i <= this.TOTAL; i++) {
      ...
      el.addEventListener('click', () => onDayClick?.(i, locked));
      grid.appendChild(el);
    }
  }
  ...
  updateDayVisual(dayNum, clicks, locked) {
    ...
    this.applyState(grid.children[dayNum - 1], dayNum, clicks, locked);
  }
  ...
  setUserInfo(username) { const userInfo = document.getElementById('user-info'); if (userInfo) userInfo.textContent = `Hola, ${username}`; }
}
