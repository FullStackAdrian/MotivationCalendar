...
router.use((error, req, res, next) => { if (error.message === 'No autorizado') return res.status(403).json({ error: error.message }); if (/no encontrado|obligatorio|inválid|límite|día de frecuencia|Tipo de frecuencia/i.test(error.message)) return res.status(400).json({ error: error.message }); return next(error); });
